...
public class KeyBinder{
    ...
    public void checkKeys(DeltaState state, Player player, TileMap grid) {
        ...
        if (state.isKeyBeingHold(controls[0])){
            ...
            player.setAppearance(player.playerState.animations[PlayerState.WALKING_LEFT]);
        ...
        if (state.isKeyBeingHold(controls[1])){
            ...
            player.setAppearance(player.playerState.animations[PlayerState.WALKING_TOP]);
            player.notify(player.toJson());
        }
        if (state.isKeyBeingHold(controls[2])){
            ...
            player.setAppearance(player.playerState.animations[PlayerState.WALKING_RIGHT]);
            player.notify(player.toJson());
        }
        if (state.isKeyBeingHold(controls[3])){
            ...
            player.setAppearance(player.playerState.animations[PlayerState.WALKING_DOWN]);
            player.notify(player.toJson());
        }
